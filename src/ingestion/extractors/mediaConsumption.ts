...
export class MediaConsumptionExtractor extends SignalExtractor {
  ...
  private fallbackExtract(data: RawSourceData): BehavioralSignal[] {
    ...
    if (EDU_PATTERN.test(content)) {
      signals.push(this.createSignal('interest', 'educational_content', 0.42, 'High consumption of educational media', data.sourceId, { category: 'learning' }));
    }
    ...
    if (eduCount > entCount * 2) {
      signals.push(this.createSignal('cognitive_trait', 'learning_focused', 0.45, 'Media consumption heavily skewed toward educational content', data.sourceId, { category: 'learning' }));
    }

    const timestamps = data.rawContent.match(/\d{4}-\d{2}-\d{2}/g) || [];
    if (timestamps.length > 10) {
      signals.push(this.createSignal('interest', 'high_media_consumption', 0.36, `High volume of watch history: ${timestamps.length} entries`, data.sourceId, { category: 'media' }));
    }
    return signals;
  }
}
