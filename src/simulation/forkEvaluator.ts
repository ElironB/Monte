// LLM Fork Evaluator - Routes between Groq (fast/cheap) and Anthropic (complex)
// Complexity threshold: >0.6 = Anthropic, with max 20 Anthropic calls per simulation

// Optional groq-sdk import (may not be installed)
// @ts-ignore - groq-sdk may not be installed, fallback to heuristic mode
...
export class ForkEvaluator {
  ...
  // Limits
  private readonly MAX_ANTHROPIC_CALLS = 20;
  ...
  constructor() {
    ...
    // Store Anthropic key if available
  ...
  async evaluateFork(
    ...
    availableAnthropicCalls: number = this.MAX_ANTHROPIC_CALLS
    ...
    const useAnthropic = complexity > this.COMPLEXITY_THRESHOLD && 
                         availableAnthropicCalls > 0 &&
                         this.anthropicApiKey !== null;
    
    try {
      if (useAnthropic) {
        return await this.callAnthropic(request, complexity);
      } else {
        return await this.callGroq(request, complexity);
      }
    } catch (error) {
  ...
  // Call Groq API (fast, cheap - for simple/medium complexity)
  private async callGroq(
    ...
    const completion = await this.groqClient.chat.completions.create({
      ...
      temperature: 0.4 + (complexity * 0.3), // Higher temp for complex decisions
      max_tokens: 150,
    ...
    this.usage.groqCalls++;
    this.usage.totalTokens += completion.usage?.total_tokens || 0;
    this.usage.estimatedCost += (completion.usage?.total_tokens || 0) * 0.0000005; // ~$0.50 per 1M tokens
    
    logger.debug({ 
      duration, 
      model: 'llama-3.1-70b',
      complexity 
    }, 'Groq evaluation completed');
  ...
  // Call Anthropic API (high quality - for complex decisions)
  private async callAnthropic(
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: 'You are a behavioral simulation engine. Analyze the persona traits and decision context carefully. Respond with valid JSON only.',
        messages: [
          {
            role: 'user',
            content: prompt,
    ...
    this.usage.anthropicCalls++;
    const tokens = (data.usage?.input_tokens || 0) + (data.usage?.output_tokens || 0);
    this.usage.totalTokens += tokens;
    this.usage.estimatedCost += tokens * 0.000001; // ~$1 per 1M tokens
...
The confidence should be 0.7-0.95 based on how clear the choice is given the traits.`;
  ...
  isAvailable(): {
    ...
    return {
      groq: this.groqClient !== null,
      anthropic: this.anthropicApiKey !== null,
      heuristicOnly: this.groqClient === null && this.anthropicApiKey === null,
    };
  }
}
...
export async function evaluateBatch(
  ...
  maxAnthropicCalls: number = 20
): Promise<LLMEvaluation[]> {
  const evaluator = forkEvaluator;
  const results: LLMEvaluation[] = [];
  let remainingAnthropic = maxAnthropicCalls;
  
  for (const request of requests) {
    const result = await evaluator.evaluateFork(request, remainingAnthropic);
    results.push(result);
...
export function estimateEvaluationCost(
  ...
  const groqTokensPerCall = 500;
  const anthropicTokensPerCall = 600;
  
  const groqCost = simpleCount * groqTokensPerCall * 0.0000005;
  const anthropicCost = Math.min(complexCount, 20) * anthropicTokensPerCall * 0.000001;
  
  return {
    groqCalls: simpleCount,
    anthropicCalls: Math.min(complexCount, 20),
    estimatedCost: groqCost + anthropicCost,
    totalTokens: simpleCount * groqTokensPerCall + Math.min(complexCount, 20) * anthropicTokensPerCall,
  };
}

// Add groq and anthropic to config schema
export const LLM_CONFIG_SCHEMA = {
  groq: {
    // @ts-ignore - process may not be defined in all environments
    apiKey: typeof process !== 'undefined' ? (process.env?.GROQ_API_KEY as string | undefined) : undefined,
  },
  anthropic: {
    // @ts-ignore - process may not be defined in all environments
    apiKey: typeof process !== 'undefined' ? (process.env?.ANTHROPIC_API_KEY as string | undefined) : undefined,
  },
};
